...
static void heapify(PQueue* pq, unsigned int root) {
  ...
  if (min_index != root) {
    Path tmp_q = pq->elements[min_index];
...
static void PQ_grow(PQueue* pq) {
  ...
  assert(pq && pq->elements);
  pq->maxsize *= 2; 
  elements = (Path*)malloc(pq->maxsize*sizeof(Path));
  if (!elements) {
...
PQueue* PQ_initialize() {
  ...
  assert(pq);
  pq->elements = (Path*)malloc(PQINITSIZE*sizeof(Path));
  if (!pq->elements) {
...
/* Set *elt to the min element in the queue and delete it from queue; 
   return value: 1 if exists a min, 0 if not   */
int PQ_extractMin(PQueue* pq, Path* elt) {
  ...
  *elt = pq->elements[0];
  ...
  PQ_DEBUG {printf("PQ_extractMin: "); printPath(*elt); printf("\n"); fflush(stdout);}
  return 1;
}
...
/************************************************************/
/* Insert */
void PQ_insert(PQueue* pq, Path elt) {
  
  unsigned int ii;
  assert(pq && pq->elements); 
 
  PQ_DEBUG {printf("PQ_insert: "); printPath(elt); printf("\n"); fflush(stdout);}
...
void PQ_deleteMinAndInsert(PQueue* pq, Path elt) {
  
  assert(pq && pq->elements); 
   PQ_DEBUG {printf("PQ_deleteMinAndinsert: "); printPath(elt); printf("\n"); fflush(stdout);}
  pq->elements[0] = elt;
  heapify(pq, 0);
}
